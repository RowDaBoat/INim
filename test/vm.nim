...
import ../src/vms/vm
import ../src/vms/nims/nimsvm
import ../src/vms/nimc/[nimcvm, compiler]
...
proc vmName(kind: VmKind): string =
  case kind
  of NimS: "NimS VM"
  of NimC: "NimC VM"


template vmTests(vmKind: VmKind) =
  suite "Reploid's " & vmName(vmKind) & " should:":
    setup:
      var vm = newVm(vmKind)
